...
export class NoteViewPage {
  ...
  getNotePanelDefaultValues(): any {
    ...
    inputText = this.getContentInputBox().getAttribute('value');
  ...
  // get note data
  getMockNote(): any {
    const note: any = { title: 'Read Angular 5 blog', content : 'Shall do at 6 pm'};
    return note;
  }
  ...
  addNoteValues(): any {
    ...
    this.getContentInputBox().sendKeys(newNote.content);
  ...
  // get text input box
  getEditContentInputBox(): ElementFinder {
    return element(by.name('editContent'));
  }
  // check text input box is present or not
  isEditTextInputBoxPresent(): promise.Promise<boolean> {
    return this.getEditContentInputBox().isPresent();
  }
  ...
  getEditNoteDefaultValues(): any {
    ...
    inputEditText = this.getEditContentInputBox().getAttribute('value');
  ...
  // get note data
  getEditMockNote(): any {
    const note: any = { title: 'Read Angular 1 blog', content : 'Shall do at 10.30 pm', state: 'not-started'};
    return note;
  }
  // set input fileds values with mock data
  editNoteValues(): any {
    ...
    this.getEditContentInputBox().clear();
    this.getEditContentInputBox().sendKeys(newNote.content);
    return Object.keys(newNote).map(key => newNote[key]);
  }
